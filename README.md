...
CLI half, closed 2026-08-06). Issue #65 now records the remaining resource/SQL parity contract:
`-n`/mod-resource scanning, `resourceExtensions.zip` discovery and digest evidence, and SQL
storages from CLI config. The exact precedence, runtime layouts, credential-safe diagnostics, and
failure meanings are documented in [`docs/compatibility/cli-resource-sql-parity.md`](docs/compatibility/cli-resource-sql-parity.md);
