...
import { createWorldlensBridge } from "@worldlens/bridge";
import type { BridgeTransport } from "@worldlens/bridge";
...
export interface RuntimeSettingsBridge {
    ...
    saveHomeAssistant(input: {
        ...
        readonly ok: boolean;
        readonly message: string;
        readonly source?: RuntimeConfiguredSource;
    }>;
...
/**
 * The addon record as it crosses the bridge. Opaque on purpose: the feature module that
 * owns its real shape is main-process code, and nothing on this side reads a field of it.
 */
...
/**
 * The Electron end of the bridge transport.
 *
 * Everything the object literal used to reach for lives here now, and nowhere else in this
 * package: `ipcRenderer` for the 330 calls and 24 event channels, the one synchronous probe,
 * `webFrame` for zoom, `webUtils` for a dropped file's real path, and `process.platform` for
 * the separator. The literal itself moved to `@worldlens/bridge` unchanged, so a browser can
 * build the same object over HTTP and the two cannot drift.
 *
 * `process.platform` is read here rather than from `node:path` because a sandboxed preload
 * has no `node:path` to import; only the limited `process` Electron injects, the context
 * bridge and `electron` itself are available.
 */
const electronTransport: BridgeTransport = {
    invoke: (channel, ...args) => ipcRenderer.invoke(channel, ...args),
    on: (channel, listener) => {
        ipcRenderer.on(channel, listener as (event: IpcRendererEvent, ...args: unknown[]) => void);
    },
    off: (channel, listener) => {
        ipcRenderer.off(channel, listener as (event: IpcRendererEvent, ...args: unknown[]) => void);
    },
    ...
    getPathForFile: (file) => {
        ...
        // file, and a drop of selected text is exactly that.
        try {
...
const bridge = createWorldlensBridge<WorldlensBridge>(electronTransport);
