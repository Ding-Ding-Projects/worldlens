...
describe("loadCiWorkflowTemplates", () => {
    it("ships managed template version 5 with stable map publication and the repository Pages base", async () => {
        ...
        const content = renderWorld?.content.replace(/\r\n?/g, "\n") ?? "";
        const expectedArtifactStep = [
            "      - name: Publish the verified map artifact",
            "        uses: actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02 # v4",
            "        if: needs.plan.outputs.single-group == 'true'",
            "        with:",
            "          name: rendered-map",
            "          path: site",
            "          retention-days: 30",
        ].join("\n");
        const basePathCalls = content
            .split("\n")
            .filter((line) => line.includes("assert-base-path.mjs"));

        expect(CI_WORKFLOW_TEMPLATE_VERSION).toBe(5);
