...
import { mergeInstalledTags, readCatalogCache, refreshOfficialCatalog, writeCatalogCache, type OllamaCatalogSnapshot } from "./catalog.js";
import { ensureOllamaRuntime, readOllamaRuntimeState, restartOllamaRuntime, stopOllamaRuntime, waitForOllamaReadiness, type OllamaProvisionProgress } from "./provision.js";
...
export function registerOllamaHandlers(ipcMain: Pick<IpcMain, "handle" | "removeHandler">, options: OllamaIpcOptions = {}): OllamaIpc {
    ...
    ipcMain.handle("ollama:generate", (event, request: unknown, operationId: unknown) => typeof request !== "object" || request === null ? [{ error: "Generation request is invalid." }] : runStream(event, operationId, (_signal) => client.generate(request as Record<string, unknown>)));
    ipcMain.handle("ollama:chat", (event, request: unknown, operationId: unknown) => typeof request !== "object" || request === null ? [{ error: "Chat request is invalid." }] : runStream(event, operationId, (_signal) => client.chat(request as Record<string, unknown>)));
