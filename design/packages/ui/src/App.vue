...
function adoptionRecord(candidate: AdoptionCandidate, hostId: string | null): ServerRecord {
    ...
    return {
        ...
        ref: hostId !== null
            ? { kind: "ssh-docker", hostId, containerRef: candidate.containerId, serverDir: candidate.serverDir ?? candidate.detected?.serverDir ?? "/data" }
            : { kind: "local-docker", containerRef: candidate.containerId, serverDir: candidate.serverDir ?? candidate.detected?.serverDir ?? "" },
        origin: "adopted",
        createdAt: now,
        updatedAt: now,
        hasRconSecret: false,
...
function reviewMcServerCandidate(candidate: AdoptionCandidate, hostId: string | null = null): void {
    ...
    mcServerAdoptPorts.value = (candidate.ports ?? []).map((port) => ({ container: port, host: null }));
