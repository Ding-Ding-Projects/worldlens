...
export const SETTINGS_FIXED = {
    ...
    "settings.engineChoice.globalPending": { en: "The global default is Automatic, but Java availability is still pending here. Keep the explicit project choice until the render probe resolves it.", yue: "全域預設係自動，但 Java 有冇仲未查到。喺 render 探測有結果之前，維持返呢個專案自己揀嗰個。" },
    "settings.engineChoice.jvmAvailable": { en: "Automatic currently selects the BlueMap original engine because Java {version} and a verified engine artifact are available.", yue: "自動而家揀咗 BlueMap 原裝引擎，因為有 Java {version}，亦都有驗證過嘅引擎檔案。" },
    "settings.engineChoice.jvmDeferred": { en: "Automatic will resolve after Java and the verified engine artifact are measured; the renderer will report the exact engine, version and capability record before it starts.", yue: "要量度過 Java 同已驗證嘅引擎檔案之後，自動先會有結果；開始之前，renderer 會報返用邊個引擎、咩版本同咩功能。" },
    "settings.engineChoice.jvmPending": { en: "Java availability is not measured on this surface yet. The project keeps its explicit BlueMap choice; rendering will verify the runtime and report any refusal before starting.", yue: "呢一版仲未量度 Java 有冇。呢個專案照用佢自己揀咗嘅 BlueMap；開始 render 之前會驗證 runtime，唔得就會報返出嚟。" },
