...
export const PROJECT_FIXED = {
    "project.destination.local": { en: "This computer", yue: "呢部電腦" },
    "project.destination.localReason": { en: "The desktop render bridge is not available.", yue: "桌面算圖 bridge 未有提供。" },
    "project.destination.docker": { en: "Local Docker", yue: "本機 Docker" },
    "project.destination.dockerReason": { en: "Docker is not an active render channel in this build or has not passed its runtime check. It will not fall back to this computer.", yue: "Docker 喺呢個 build 唔係有效算圖渠道，或者未通過 runtime 檢查；唔會偷偷返本機。" },
    "project.destination.remote": { en: "SSH remote machine", yue: "SSH 遠端機" },
    "project.destination.remoteBridge": { en: "The SSH render bridge is not available.", yue: "SSH 算圖 bridge 未有提供。" },
    "project.destination.remoteTarget": { en: "Add and select a remote machine first.", yue: "先加入同揀一部遠端機。" },
    "project.destination.remotePreflight": { en: "Run the SSH, host-key, Docker and disk checks before sending a world.", yue: "送出世界之前，要先完成 SSH、host key、Docker 同磁碟檢查。" },
    "project.destination.github": { en: "GitHub Actions", yue: "GitHub Actions" },
    "project.destination.githubReason": { en: "The full GitHub Actions render wizard is not available here.", yue: "完整 GitHub Actions 算圖 wizard 喺呢度未有提供。" },
    "project.destination.import": { en: "Import rendering project", yue: "匯入算圖 project" },
    "project.destination.publish": { en: "Publish existing render", yue: "發佈已有算圖" },
    "project.destination.renderDocker": { en: "Render in local Docker", yue: "用本機 Docker 算圖" },
    "project.destination.renderRemote": { en: "Render on the SSH machine", yue: "喺 SSH 遠端機算圖" },
    "project.destination.renderLocal": { en: "Render on this computer", yue: "喺呢部電腦算圖" },
    "project.destination.choose": { en: "Choose where to render", yue: "揀喺邊度算圖" },
    "project.destination.title": { en: "Choose where to render", yue: "揀喺邊度算圖" },
    "project.destination.menuLabel": { en: "Render destinations", yue: "算圖目的地" },
    "project.import.title": { en: "Import a rendering project", yue: "匯入算圖 project" },
    "project.import.lede": { en: "Choose a local project, fetch a verified project from an SSH machine, or review the archive route. The source is read-only; the imported local folder is inspected by the same project schema and history path as every other project.", yue: "揀本機 project、由 SSH 機攞返已核實嘅 project，或者查看 archive 路線。來源只讀；匯入嘅本機資料夾會用同一套 project schema 同 history 路線檢查。" },
    "project.import.localTitle": { en: "Local world or project file", yue: "本機世界或者 project 檔案" },
    "project.import.localHint": { en: "Browse for a world folder or the exact worldlens.project.json file. The file is parsed and validated before it opens.", yue: "瀏覽世界資料夾或者指定嘅 worldlens.project.json；開啟之前會先解析同驗證。" },
    "project.import.folder": { en: "Browse world folder", yue: "瀏覽世界資料夾" },
    "project.import.file": { en: "Browse project file", yue: "瀏覽 project 檔案" },
    "project.import.localUnavailable": { en: "Local import needs the desktop file picker and the verified project host.", yue: "本機匯入需要桌面檔案選擇器同已核實嘅 project host。" },
    "project.import.sshTitle": { en: "SSH remote machine", yue: "SSH 遠端機" },
    "project.import.archiveTitle": { en: "Archive or manifest", yue: "Archive 或 manifest" },
    "project.import.archive": { en: "Import archive or manifest", yue: "匯入 archive 或 manifest" },
    "project.import.archiveUnavailable": { en: "Safe archive extraction and manifest verification are not exposed by this build yet. Use a world folder, a project file, or the SSH fetch above; those routes validate before import.", yue: "呢個 build 暫時未有安全 archive 解包同 manifest 驗證 bridge。請用世界資料夾、project 檔案，或者上面嘅 SSH fetch；嗰幾條路線匯入前都會驗證。" },
    "project.import.invalid": { en: "The selected folder does not contain a readable worldlens.project.json ({kind}).", yue: "揀嘅資料夾冇可讀嘅 worldlens.project.json（{kind}）。" },
    "project.import.pickFolder": { en: "Choose a world folder containing worldlens.project.json", yue: "揀一個有 worldlens.project.json 嘅世界資料夾" },
    "project.import.pickFile": { en: "Choose worldlens.project.json", yue: "揀 worldlens.project.json" },
    "project.import.cancel": { en: "Cancel", yue: "取消" },
    "project.render.pages": { en: "Publish this project to GitHub Pages", yue: "將呢個 project 發佈到 GitHub Pages" },
    "project.render.pagesHint": { en: "When enabled, the existing Pages flow keeps a verified render selected and continues setup. Turning it off stops automatic publication but does not delete an existing site.", yue: "開啟之後，已有 Pages flow 會保留已核實嘅算圖並繼續設定；關閉只會停自動發佈，唔會刪除已有網站。" },
    "project.render.pagesPending": { en: "Pages setup is pending. The project will not claim enabled until the existing guided publication flow accepts it.", yue: "Pages 設定仲等緊；已有 guided publication flow 未接受之前，project 唔會話自己已開啟。" },
    "project.render.pagesFailed": { en: "Pages setup failed. Automatic publication remains off until it succeeds.", yue: "Pages 設定失敗；成功之前自動發佈仍然關閉。" },
    "project.render.pagesPublished": { en: "Pages publication is enabled for this project.", yue: "呢個 project 已開啟 Pages 發佈。" },
