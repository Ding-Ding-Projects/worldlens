...
<template>
    <section class="wl-release-ledger" aria-labelledby="release-ledger-title">
        <header class="wl-release-ledger__header"><div><p class="wl-release-ledger__eyebrow">{{ t("releaseLedger.eyebrow", "Release evidence") }}</p><h2 id="release-ledger-title">{{ t("releaseLedger.title", "Phase release ledger") }}</h2><p>{{ t("releaseLedger.lede", "Read-only evidence for each completed implementation phase. A local build never becomes a cloud verdict by wishful thinking.") }}</p></div><div class="wl-release-ledger__actions"><VBtn variant="tonal" :prepend-icon="mdiRefresh" :loading="loading" @click="refresh">{{ t("releaseLedger.refresh", "Refresh evidence") }}</VBtn><VBtn variant="text" :prepend-icon="mdiContentCopy" :disabled="!readout" @click="copy">{{ t("releaseLedger.copy", "Copy shown") }}</VBtn><VBtn variant="text" :prepend-icon="mdiDownload" :disabled="!readout" @click="download">{{ t("releaseLedger.export", "Export Markdown") }}</VBtn></div></header>
        <p v-if="loading" class="wl-release-ledger__state" role="status"><VProgressCircular indeterminate size="18" width="2" aria-hidden="true" />{{ t("releaseLedger.loading", "Reading the live release ledger…") }}</p>
        <p v-else-if="failure" class="wl-release-ledger__state wl-release-ledger__state--error" role="status">{{ failure }}</p>
        <template v-else-if="readout"><ConfigSearchField v-model="query" v-model:regex="regex" v-model:flags="flags" class="wl-release-ledger__search" :label="t('releaseLedger.search', 'Search phases, commits and releases')" :sample="entries.map((entry) => `${entry.phase} ${entry.releaseTag ?? ''}`).join(' | ')" :summary="t('releaseLedger.count', { shown: shown.length, total: entries.length }, '{shown} of {total} phases')" /><p class="wl-release-ledger__source">{{ t("releaseLedger.source", { at: readout.readAt }, "Live bridge read at {at}") }}</p><p v-if="shown.length === 0" class="wl-release-ledger__state" role="status">{{ t("releaseLedger.empty", "No phase matches this search. Nothing has been fabricated to fill the gap.") }}</p><ol v-else class="wl-release-ledger__list"><li v-for="entry in shown" :key="entry.id" class="wl-release-ledger__row"><div class="wl-release-ledger__row-head"><div><h3>{{ entry.phase }}</h3><p class="wl-release-ledger__sha">{{ entry.integrationSha }}</p></div><VChip :color="entry.verification === 'verified' ? 'success' : entry.verification === 'failed' ? 'error' : 'warning'" size="small"><VIcon :icon="statusIcon(entry.verification)" size="16" start />{{ statusLabel(entry.verification) }}</VChip></div><dl class="wl-release-ledger__facts"><div><dt>{{ t("releaseLedger.release", "Release") }}</dt><dd><a v-if="entry.releaseUrl" :href="entry.releaseUrl" target="_blank" rel="noopener noreferrer">{{ entry.releaseTag }}</a><span v-else>{{ t("releaseLedger.notPublished", "not published") }}</span></dd></div><div><dt>{{ t("releaseLedger.workflow", "Workflow") }}</dt><dd><a v-if="entry.workflowUrl" :href="entry.workflowUrl" target="_blank" rel="noopener noreferrer">{{ entry.workflowRun }}</a><span v-else>{{ entry.workflowRun ?? t("releaseLedger.notReported", "not reported") }}</span> · {{ entry.workflowState }}</dd></div><div><dt>{{ t("releaseLedger.timing", "Timing") }}</dt><dd>{{ entry.startedAt ?? t("releaseLedger.notReported", "not reported") }} → {{ entry.completedAt ?? t("releaseLedger.notReported", "not reported") }} · {{ entry.duration ?? t("releaseLedger.notReported", "not reported") }}</dd></div><div><dt>{{ t("releaseLedger.codeName", "Code name") }}</dt><dd>{{ entry.codeName ?? t("releaseLedger.none", "none recorded") }}</dd></div><div><dt>{{ t("releaseLedger.lines", "Lines") }}</dt><dd>{{ entry.lineCount ?? t("releaseLedger.notReported", "not reported") }}</dd></div><div><dt>{{ t("releaseLedger.catalog", "Public catalog") }}</dt><dd><a v-if="entry.catalogUrl" :href="entry.catalogUrl" target="_blank" rel="noopener noreferrer">{{ t("releaseLedger.openCatalog", "Open catalog evidence") }}</a><span v-else>{{ t("releaseLedger.notReported", "not reported") }}</span></dd></div></dl><p class="wl-release-ledger__note">{{ entry.verificationNote }}</p><ul v-if="entry.assets.length" class="wl-release-ledger__assets"><li v-for="asset in entry.assets" :key="asset.name"><strong>{{ asset.name }}</strong> · {{ bytes(asset.bytes) }}<span v-if="asset.sha256"> · SHA-256 {{ asset.sha256 }}</span></li></ul></li></ol></template>
    </section>
</template>
