            ...
            <tbody>
                <tr v-for="(row, index) in rows" :key="index">
                    <td v-for="column in control.columns" :key="column.key">
                        <ConfigControl :control="column.control" :model-value="row[column.key] ?? ''" :label="`${column.label} ${index + 1}`" :disabled="isDisabled" @update:model-value="(value) => update(index, column.key, value)" />
                    </td>
                    <td><VBtn size="small" variant="text" :disabled="isDisabled" @click="remove(index)">Remove</VBtn></td>
                </tr>
            </tbody>
        </VTable>
        <VBtn size="small" variant="tonal" :disabled="isDisabled" @click="add">Add record</VBtn>
    </div>
</template>
