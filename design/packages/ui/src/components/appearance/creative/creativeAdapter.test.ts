...
import {
    ...
    creativeDocumentFor,
    resetCreativeDocumentFor,
} from "./creativeAdapter.js";
...
describe("creative adapter integration", () => {
    it("reads only validated documents from the core preserved appearance record", () => {
        ...
        const target = {
            ...emptyRecord(),
            preserved: { creativeDocument: document },
        };
        state.elements.target = target;
            ...
            creativeDocumentFor(
                {
                    ...state,
                    elements: {
                        target: { ...target, preserved: { creativeDocument: { bad: true } } },
                    },
                },
                "target",
            ),
    ...
    it("applies through the core commit seam and survives reload, then resets cleanly", () => {
        ...
        expect(creativeDocumentFor(appearanceState().value, "creative-test-target")).toEqual(
            document,
        );
        reloadAppearance();
        expect(creativeDocumentFor(appearanceState().value, "creative-test-target")).toEqual(
    ...
    it("migrates a persisted version 1 document and writes the migrated value through core history", () => {
        ...
        state.elements["legacy-migration-target"] = {
            typography: {},
            surface: {},
            inherit: "",
            preserved: { creativeDocument: legacy },
        } as unknown as AppearanceRecord;
        const migrated = creativeDocumentFor(state, "legacy-migration-target");
        expect(migrated?.version).toBe(2);
        expect(
            creativeDocumentFor(appearanceState().value, "legacy-migration-target")?.canvas.grid,
        ).toBeDefined();
        expect(appearanceState().value.elements["legacy-migration-target"]?.states).toEqual({});
        resetCreativeDocumentFor("legacy-migration-target");
        expect(appearanceState().value.elements["legacy-migration-target"]).toBeUndefined();
    });
});
