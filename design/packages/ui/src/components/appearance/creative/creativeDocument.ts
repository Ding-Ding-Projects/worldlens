...
function snapCoordinate(document: CreativeAppearanceDocument, value: number): number {
    return document.canvas.grid.snap
        ? Math.round(value / document.canvas.grid.size) * document.canvas.grid.size
        : value;
}
...
export function createTextLayer(overrides: Partial<CreativeTextLayer> = {}): CreativeTextLayer {
    return {
        id: overrides.id ?? id("text"),
        name: cleanName(overrides.name ?? "Text", "Text"),
        kind: "text",
        parentId: overrides.parentId ?? null,
        visible: overrides.visible ?? true,
        opacity: clamp(overrides.opacity ?? 1, 0, 1),
        blendMode: overrides.blendMode ?? "normal",
        clipped: overrides.clipped ?? false,
        clipSourceId: overrides.clipSourceId ?? null,
        locked: overrides.locked ?? false,
        mask: overrides.mask ?? null,
        effects: clone(overrides.effects ?? DEFAULT_EFFECTS),
        text: (overrides.text ?? "Double-click to edit").slice(0, CREATIVE_LIMITS.maxTextLength),
        x: overrides.x ?? 96,
        y: overrides.y ?? 140,
        ...
        fill: overrides.fill ?? "#f8f9fa",
        typography: clone(
            overrides.typography ?? ({ ...DEFAULT_TYPOGRAPHY } as Partial<TypographySpec>),
        ),
    };
}

export function createVectorLayer(
    overrides: Partial<CreativeVectorLayer> = {},
): CreativeVectorLayer {
    return {
        id: overrides.id ?? id("shape"),
        name: cleanName(overrides.name ?? "Shape", "Shape"),
        kind: "vector",
        parentId: overrides.parentId ?? null,
        visible: overrides.visible ?? true,
        opacity: clamp(overrides.opacity ?? 1, 0, 1),
...
export function createGradientLayer(
    overrides: Partial<CreativeGradientLayer> = {},
): CreativeGradientLayer {
    return {
        id: overrides.id ?? id("gradient"),
        name: cleanName(overrides.name ?? "Gradient", "Gradient"),
        kind: "gradient",
        parentId: overrides.parentId ?? null,
        visible: overrides.visible ?? true,
        opacity: clamp(overrides.opacity ?? 1, 0, 1),
...
export function createRasterLayer(
    ...
    return {
        id: id("image"),
        name: cleanName(overrides.name ?? "Imported image", "Imported image"),
        kind: "raster",
        parentId: overrides.parentId ?? null,
        visible: true,
        opacity: 1,
...
export function commitCreativeChange(
    ...
    return {
        format: CREATIVE_DOCUMENT_FORMAT,
        version: CREATIVE_DOCUMENT_VERSION,
        canvas: clone(next.canvas),
        layers: clone(next.layers),
        selectedLayerIds: [...next.selectedLayerIds],
        history,
        historyCursor: history.length - 1,
        presets: clone(document.presets),
        logo: clone(document.logo),
    };
}
...
export function updateCreativeLayer(
    ...
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers, selectedLayerIds: document.selectedLayerIds },
        action,
    );
}

export function addCreativeLayer(
    document: CreativeAppearanceDocument,
    layer: CreativeLayer,
    action = "add layer",
): CreativeAppearanceDocument {
...
export function removeCreativeLayers(
    document: CreativeAppearanceDocument,
    ids: readonly string[],
): CreativeAppearanceDocument {
    ...
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers: keep, selectedLayerIds: [] },
        "delete layers",
    );
}

export function reorderCreativeLayer(
    document: CreativeAppearanceDocument,
    layerId: string,
    direction: -1 | 1,
): CreativeAppearanceDocument {
    ...
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers: next, selectedLayerIds: [layerId] },
        "reorder layer",
    );
}

export function groupCreativeLayers(
    document: CreativeAppearanceDocument,
    ids: readonly string[],
): CreativeAppearanceDocument {
    if (
        ids.length === 0 ||
        document.layers.filter((layer) => layer.kind === "group").length >=
            CREATIVE_LIMITS.maxGroups
    )
        return document;
    ...
    const layers = [
        ...
        ...document.layers.map((layer) =>
            selected.has(layer.id) ? { ...layer, parentId: group.id } : layer,
        ),
    ];
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers, selectedLayerIds: [group.id] },
        "group layers",
    );
}

export function resetCreativeLayer(
    document: CreativeAppearanceDocument,
    layerId: string,
): CreativeAppearanceDocument {
    ...
    const reset =
        layer.kind === "group"
            ...
            : layer.kind === "raster"
              ...
              : layer.kind === "text"
                ? createTextLayer({ id: layer.id, name: layer.name, parentId: layer.parentId })
                : layer.kind === "vector"
                  ? createVectorLayer({ id: layer.id, name: layer.name, parentId: layer.parentId })
                  : layer.kind === "gradient"
                    ? createGradientLayer({
                          id: layer.id,
                          name: layer.name,
...
export function duplicateCreativeLayers(
    document: CreativeAppearanceDocument,
    ids: readonly string[],
): CreativeAppearanceDocument {
    const selected = new Set(ids);
    ...
    const copies = sourceLayers.map(
        (layer) =>
            ({
                ...
                parentId:
                    layer.parentId !== null && remap.has(layer.parentId)
                        ? remap.get(layer.parentId)!
                        : layer.parentId,
                x: "x" in layer ? layer.x + 24 : undefined,
                y: "y" in layer ? layer.y + 24 : undefined,
            }) as CreativeLayer,
    );
    if (copies.length === 0 || document.layers.length + copies.length > CREATIVE_LIMITS.maxLayers)
        return document;
    return commitCreativeChange(
        document,
        {
...
export function ungroupCreativeLayers(
    document: CreativeAppearanceDocument,
    groupId: string,
): CreativeAppearanceDocument {
    ...
    const layers = document.layers
        ...
        .map((layer) =>
            layer.parentId === groupId ? { ...layer, parentId: group.parentId } : layer,
        );
    return commitCreativeChange(
        document,
        {
            ...
            selectedLayerIds: layers
                .filter((layer) => layer.parentId === group.parentId)
                .map((layer) => layer.id),
        },
        "ungroup layers",
    );
...
export function setCreativeCanvas(
    document: CreativeAppearanceDocument,
    patch: Partial<CreativeCanvas>,
    action = "adjust canvas",
): CreativeAppearanceDocument {
    const next = {
        ...document.canvas,
        ...clone(patch),
        width: clamp(patch.width ?? document.canvas.width, 1, 4096),
        height: clamp(patch.height ?? document.canvas.height, 1, 4096),
    };
    return commitCreativeChange(
        document,
        { canvas: next, layers: document.layers, selectedLayerIds: document.selectedLayerIds },
        action,
    );
}

export function resetCreativeCanvas(
    document: CreativeAppearanceDocument,
): CreativeAppearanceDocument {
    return setCreativeCanvas(document, canvasDefaults(1024, 768, "#202124"), "reset canvas");
}

export function alignCreativeLayers(
    document: CreativeAppearanceDocument,
    ids: readonly string[],
    axis: "left" | "center" | "right" | "top" | "middle" | "bottom",
): CreativeAppearanceDocument {
    const selected = document.layers.filter(
        (layer): layer is Exclude<CreativeLayer, { kind: "group" }> =>
            ids.includes(layer.id) && "x" in layer && "y" in layer,
    );
    ...
    const layers = document.layers.map((layer) => {
        ...
        const x = snapCoordinate(
            ...
            axis === "left"
                ? minX
                : axis === "center"
                  ? (minX + maxRight - layer.width) / 2
                  : axis === "right"
                    ? maxRight - layer.width
                    : layer.x,
        );
        const y = snapCoordinate(
            ...
            axis === "top"
                ? minY
                : axis === "middle"
                  ? (minY + maxBottom - layer.height) / 2
                  : axis === "bottom"
                    ? maxBottom - layer.height
                    : layer.y,
        );
    ...
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers, selectedLayerIds: [...ids] },
        `align ${axis}`,
    );
}

export function distributeCreativeLayers(
    document: CreativeAppearanceDocument,
    ids: readonly string[],
    axis: "horizontal" | "vertical",
): CreativeAppearanceDocument {
    const selected = document.layers
        .filter(
            (layer): layer is Exclude<CreativeLayer, { kind: "group" }> =>
                ids.includes(layer.id) && "x" in layer && "y" in layer,
        )
    ...
    const layers = document.layers.map((layer) =>
        positions.has(layer.id)
            ? ({
                  ...layer,
                  ...(axis === "horizontal"
                      ? { x: positions.get(layer.id) }
                      : { y: positions.get(layer.id) }),
              } as CreativeLayer)
            : layer,
    );
    return commitCreativeChange(
        document,
        { canvas: document.canvas, layers, selectedLayerIds: [...ids] },
        axis === "horizontal" ? "distribute horizontal layers" : "distribute vertical layers",
    );
}

export function setCreativeLogo(
    document: CreativeAppearanceDocument,
    logo: Partial<CreativeAppearanceDocument["logo"]>,
): CreativeAppearanceDocument {
    ...
    const next = {
        ...
        logo: {
            ...mergedLogo,
            ownership: mergedLogo.target === "app-logo" ? mergedLogo.ownership : null,
        },
    };
    const entry = snapshot(
        "adjust logo composition",
        next.canvas,
        next.layers,
        next.selectedLayerIds,
        next.presets,
    ...
    const history = [...document.history.slice(0, document.historyCursor + 1), entry].slice(
        -CREATIVE_LIMITS.maxHistory,
    );
    return { ...next, history, historyCursor: history.length - 1 };
}

export function saveCreativePreset(
    document: CreativeAppearanceDocument,
    name: string,
): CreativeAppearanceDocument {
    const preset = {
...
export function resetCreativeDocument(
    document: CreativeAppearanceDocument,
): CreativeAppearanceDocument {
    const next = createCreativeDocument();
    return commitCreativeChange(
        { ...next, presets: document.presets, logo: document.logo },
        { canvas: next.canvas, layers: [], selectedLayerIds: [] },
        "reset creative document",
    );
}
...
const COLOR_RE =
    /^(?:#[0-9a-f]{3,8}|(?:rgb|rgba|hsl|hsla|hwb|oklab|oklch|lab|lch|color)\([^\n]{1,180}\)|[a-z]{1,32})$/i;

function validColor(value: unknown): value is string {
    return (
        typeof value === "string" && value.length <= 192 && (value === "" || COLOR_RE.test(value))
    );
}
...
function validEffects(value: unknown): value is CreativeEffectStack {
    if (
        !isObject(value) ||
        !validNumber(value.blur, 0, 128) ||
        !validNumber(value.brightness, 0, 8) ||
        !validNumber(value.contrast, 0, 8) ||
        !validNumber(value.saturation, 0, 8) ||
        !validNumber(value.hue, -360, 360) ||
        !validNumber(value.grayscale, 0, 1) ||
        !validNumber(value.sepia, 0, 1) ||
        !validNumber(value.invert, 0, 1) ||
        !isObject(value.shadow) ||
        !isObject(value.innerGlow) ||
        !isObject(value.outerGlow)
    )
        return false;
    return (
        validNumber(value.shadow.x, -4096, 4096) &&
        validNumber(value.shadow.y, -4096, 4096) &&
        validNumber(value.shadow.blur, 0, 256) &&
        validColor(value.shadow.color) &&
        validNumber(value.innerGlow.radius, 0, 256) &&
        validColor(value.innerGlow.color) &&
        validNumber(value.outerGlow.radius, 0, 256) &&
        validColor(value.outerGlow.color)
    );
}
...
function validLayer(
    ...
    ids.add(layer.id);
    if (
        layer.name.length > CREATIVE_LIMITS.maxNameLength ||
        !["group", "raster", "vector", "text", "gradient"].includes(layer.kind)
    )
        return false;
    ...
    if (
        ...
        (layer.clipSourceId !== null && typeof layer.clipSourceId !== "string") ||
        !validNumber(layer.opacity, 0, 1) ||
        !BLEND_MODES.includes(layer.blendMode as CreativeBlendMode) ||
        !validEffects(layer.effects)
    )
        return false;
    if (
        layer.mask !== null &&
        (!isObject(layer.mask) ||
            typeof layer.mask.enabled !== "boolean" ||
            !["rectangle", "ellipse"].includes(String(layer.mask.kind)) ||
            !validNumber(layer.mask.x) ||
            !validNumber(layer.mask.y) ||
            !validNumber(layer.mask.width, 1, canvas.width) ||
            !validNumber(layer.mask.height, 1, canvas.height) ||
            !validNumber(layer.mask.feather, 0, 256))
    )
        return false;
    if (layer.kind === "group") return true;
    if (
        !validNumber(layer.x, -canvas.width, canvas.width * 2) ||
        !validNumber(layer.y, -canvas.height, canvas.height * 2) ||
        !validNumber(layer.width, 1, canvas.width * 2) ||
        !validNumber(layer.height, 1, canvas.height * 2) ||
        (layer.kind !== "gradient" && !validNumber(layer.rotation, -3600, 3600)) ||
        !validNumber(layer.scaleX, 0.01, 100) ||
        !validNumber(layer.scaleY, 0.01, 100)
    )
        return false;
    if (
        layer.kind === "text" &&
        (typeof layer.text !== "string" ||
            layer.text.length > CREATIVE_LIMITS.maxTextLength ||
            !validColor(layer.fill) ||
            !isObject(layer.typography))
    )
        return false;
    ...
    if (
        layer.kind === "vector" &&
        (!validColor(layer.fill) ||
            !validColor(layer.stroke) ||
            !validNumber(layer.strokeWidth, 0, 64) ||
            !["rect", "ellipse", "line"].includes(String(layer.shape)))
    )
        return false;
    if (
        layer.kind === "gradient" &&
        (!validNumber(layer.angle, -3600, 3600) ||
            !Array.isArray(layer.stops) ||
            layer.stops.length < 2 ||
            layer.stops.length > 32 ||
            !layer.stops.every(
                (stop) =>
                    isObject(stop) && validNumber(stop.offset, 0, 1) && validColor(stop.color),
            ))
    )
        return false;
    return true;
...
function validCanvas(value: unknown): value is CreativeCanvas {
    if (
        !isObject(value) ||
        !validNumber(value.width, 1, 4096) ||
        !validNumber(value.height, 1, 4096) ||
        !validColor(value.background) ||
        value.width * value.height > CREATIVE_LIMITS.maxCanvasPixels
    )
        return false;
    const guideIds = new Set<string>();
    return (
        isObject(value.crop) &&
        validNumber(value.crop.top, 0, value.height) &&
        validNumber(value.crop.right, 0, value.width) &&
        validNumber(value.crop.bottom, 0, value.height) &&
        validNumber(value.crop.left, 0, value.width) &&
        value.crop.top + value.crop.bottom < value.height &&
        value.crop.left + value.crop.right < value.width &&
        typeof value.rulers === "boolean" &&
        isObject(value.grid) &&
        typeof value.grid.enabled === "boolean" &&
        validNumber(value.grid.size, 1, 512) &&
        typeof value.grid.snap === "boolean" &&
        isObject(value.safeArea) &&
        validNumber(value.safeArea.inset, 0, 512) &&
        typeof value.safeArea.enabled === "boolean" &&
        Array.isArray(value.guides) &&
        value.guides.length <= 128 &&
        value.guides.every(
            (guide) =>
                isObject(guide) &&
                ...
                ["x", "y"].includes(String(guide.axis)) &&
                    validNumber(guide.position, -4096, 8192)),
        )
    );
}

function validLayerGraph(
    value: unknown,
    canvas: CreativeCanvas,
): value is readonly CreativeLayer[] {
    ...
    for (const layer of value) {
        if (layer.clipSourceId !== null) {
            ...
            if (
                !source ||
                source.id === layer.id ||
                source.parentId !== layer.parentId ||
                source.kind === "group"
            )
                return false;
        }
...
function validLogoComposition(value: unknown): boolean {
    if (
        ...
        !["app-logo", "appearance-target"].includes(String(value.target)) ||
        (value.activeVariantId !== null && typeof value.activeVariantId !== "string") ||
        (value.ownership !== null &&
            (!isObject(value.ownership) ||
                typeof value.ownership.token !== "string" ||
                typeof value.ownership.revision !== "number" ||
                !Number.isInteger(value.ownership.revision) ||
                value.ownership.revision < 0)) ||
        !isObject(value.presentation) ||
        typeof value.presentation.presetId !== "string" ||
        !isObject(value.presentation.crop) ||
        !validNumber(value.presentation.crop.top, 0, 40) ||
        !validNumber(value.presentation.crop.right, 0, 40) ||
        !validNumber(value.presentation.crop.bottom, 0, 40) ||
        !validNumber(value.presentation.crop.left, 0, 40) ||
        !["fill", "contain"].includes(String(value.presentation.fit)) ||
        !isObject(value.presentation.focalPoint) ||
        !validNumber(value.presentation.focalPoint.x, 0, 100) ||
        !validNumber(value.presentation.focalPoint.y, 0, 100) ||
        !["transparent", "solid"].includes(String(value.presentation.background)) ||
    ...
    if (
        !value.variants.every(
            (variant) =>
                ...
                validNumber(variant.width, 1, 2048) &&
                    validNumber(variant.height, 1, 2048) &&
                    validLogoVariantDataUrl(variant.dataUrl)),
        )
    )
        return false;
...
export function validateCreativeDocument(value: unknown): value is CreativeAppearanceDocument {
    if (
        !isObject(value) ||
        value.format !== CREATIVE_DOCUMENT_FORMAT ||
        value.version !== CREATIVE_DOCUMENT_VERSION
    )
        return false;
    ...
    if (
        ...
        value.history.length > CREATIVE_LIMITS.maxHistory ||
        !validNumber(value.historyCursor, 0, Math.max(0, value.history.length - 1))
    )
        return false;
    if (
        !Array.isArray(value.presets) ||
        value.presets.length > 32 ||
        !value.presets.every(
            (preset) =>
                isObject(preset) &&
                typeof preset.id === "string" &&
                typeof preset.name === "string" &&
                validCanvas(preset.canvas) &&
                validLayerGraph(preset.layers, preset.canvas),
        )
    )
        return false;
    ...
    return value.history.every(
        (entry) =>
            ...
            validCanvas(entry.canvas) &&
            validLayerGraph(entry.layers, entry.canvas) &&
            selectedIdsValid(entry.layers, entry.selectedLayerIds) &&
            ...
            validLogoComposition(entry.logo),
    );
}

export function importCreativeDocument(text: string): CreativeImportResult {
    if (text.length > 12 * 1024 * 1024)
        throw new Error("The creative document is larger than the 12 MB safety limit.");
    let parsed: unknown;
    try {
        parsed = JSON.parse(text);
    } catch {
        throw new Error("The creative document is not valid JSON.");
    }
    const migrated = migrateCreativeDocument(parsed);
    if (!validateCreativeDocument(migrated))
        throw new Error("The creative document does not match the supported bounded format.");
    return {
        document: clone(migrated),
        warnings:
            parsed !== migrated
                ? ["This document was migrated from creative format version 1."]
                : [],
    };
}
...
function migrateCanvas(raw: unknown): Record<string, unknown> {
    ...
    return {
        ...
        grid: isObject(value.grid)
            ? {
                  enabled: Boolean(value.grid.enabled),
                  size: value.grid.size ?? 16,
                  snap: Boolean(value.grid.snap),
              }
            : { enabled: false, size: 16, snap: false },
...
function migrateLogo(raw: unknown): Record<string, unknown> {
    ...
    return {
        ...clone(DEFAULT_LOGO),
        ...value,
        ownership: isObject(value.ownership) ? value.ownership : null,
        presentation: isObject(value.presentation)
            ? { ...clone(DEFAULT_LOGO.presentation), ...value.presentation }
            : clone(DEFAULT_LOGO.presentation),
        safeArea: isObject(value.safeArea) ? value.safeArea : clone(DEFAULT_LOGO.safeArea),
        variants,
        activeVariantId:
            typeof value.activeVariantId === "string"
                ? value.activeVariantId
                : isObject(variants[0]) && typeof variants[0].id === "string"
                  ? variants[0].id
                  : null,
    };
}
...
export function migrateCreativeDocument(value: unknown): unknown {
    if (
        !isObject(value) ||
        value.format !== CREATIVE_DOCUMENT_FORMAT ||
        value.version !== LEGACY_CREATIVE_DOCUMENT_VERSION
    )
        return value;
    const migratedLayers = Array.isArray(value.layers) ? value.layers.map(migrateLayer) : [];
    const migrateHistory = Array.isArray(value.history)
...
export function exportCreativeDocument(document: CreativeAppearanceDocument): string {
    if (!validateCreativeDocument(document))
        throw new Error("The creative document is not valid and cannot be exported.");
    return JSON.stringify(document, null, 2);
}

export function safeCreativeCapabilities(
    capabilities: Partial<CreativeAppearanceCapabilities> = {},
): CreativeAppearanceCapabilities {
    return {
        ...DEFAULT_CREATIVE_CAPABILITIES,
        ...capabilities,
        reasonByCapability: { ...capabilities.reasonByCapability },
    };
}

export function validateCreativeAssetBytes(bytes: Uint8Array, mime = ""): CreativeAssetValidation {
    if (bytes.byteLength > CREATIVE_LIMITS.maxAssetBytes)
        throw new Error("The image is larger than the 8 MB safety limit.");
    if (
        bytes.length >= 24 &&
        bytes[0] === 0x89 &&
        bytes[1] === 0x50 &&
        ...
        if (width * height > CREATIVE_LIMITS.maxAssetPixels)
            throw new Error("The image exceeds the 16 megapixel safety limit.");
    ...
    if (bytes.length >= 3 && bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff) {
        ...
        if (width > 0 && height > 0 && width * height > CREATIVE_LIMITS.maxAssetPixels)
            throw new Error("The image exceeds the 16 megapixel safety limit.");
        return { format: "jpeg", width, height, frames: 1, mime: mime || "image/jpeg" };
    }
    const text = new TextDecoder().decode(bytes).trimStart();
    if (text.startsWith("<svg") && !/<script|foreignObject|https?:\/\//i.test(text))
        return { format: "svg", width: 0, height: 0, frames: 1, mime: mime || "image/svg+xml" };
    throw new Error("The selected file is not a supported PNG, JPEG, WebP, or safe SVG image.");
}

export async function importCreativeAsset(
    file: File,
    document: CreativeAppearanceDocument,
): Promise<CreativeAppearanceDocument> {
    if (file.size > CREATIVE_LIMITS.maxAssetBytes)
        throw new Error("The image is larger than the 8 MB safety limit.");
    const bytes = new Uint8Array(await file.arrayBuffer());
    const validation = validateCreativeAssetBytes(bytes, file.type);
    if (validation.frames > CREATIVE_LIMITS.maxAssetFrames)
        throw new Error("Animated images are not supported.");
    let binary = "";
