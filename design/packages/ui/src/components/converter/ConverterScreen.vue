...
import { VAlert, VBtn, VCard, VCheckbox, VChip, VDivider, VProgressLinear, VSelect, VTextarea, VTextField } from "vuetify/components";
...
const pdfOverwriteConfirmation = ref("");
const pdfTitle = ref("");
const pdfAuthor = ref("");
...
const visiblePdfOperations = computed(() => pdfOperations.filter((item) => item.includes(pdfOperationSearch.value.trim().toLowerCase())));
...
async function pickPdfInput(): Promise<void> { const path = await globalThis.window?.worldlens?.dialog.pickFile({ title: "Choose a PDF input", extensions: ["pdf"] }); if (!path) return; const current = source.value ? source.value.split("\n").filter(Boolean) : []; if (!current.includes(path)) current.push(path); source.value = current.join("\n"); inspection.value = await bridge?.inspect(path) ?? null; }
async function runPdf(): Promise<void> {
    if (!bridge || !source.value) return;
    const inputs = source.value.split(/\r?\n/).map((value) => value.trim()).filter(Boolean);
    const output = pdfOutput.value || targetPath.value || (pdfOperation.value === "inspect" ? "inspect-only.pdf" : null);
    if (!output) { status.value = "Choose an output file or output folder first."; return; }
    const request = { operation: pdfOperation.value, inputs, output, overwrite: pdfOverwrite.value, ...(pdfOverwriteConfirmation.value ? { overwriteConfirmation: pdfOverwriteConfirmation.value } : {}), ...(parsedPdfPages.value.length ? { pages: parsedPdfPages.value } : {}), ...(pdfOperation.value === "rotate" ? { rotation: pdfRotation.value } : {}), ...(parsedPdfOutputs.value.length ? { outputs: parsedPdfOutputs.value } : {}), ...(pdfOperation.value === "metadata" ? { metadata: { title: pdfTitle.value, author: pdfAuthor.value, subject: pdfSubject.value, keywords: pdfKeywords.value, creator: pdfCreator.value, producer: pdfProducer.value } } : {}) };
    const result = await bridge.pdf(request);
    status.value = result.message;
    if (result.ok) { const targets = result.outputs?.length ? result.outputs.join(", ") : result.output ?? output; saveHistory({ source: inputs.join(", "), target: targets, message: `PDF ${pdfOperation.value} completed` }); }
}
...
<template>
    <section class="mb-converter" data-test="converter-screen" aria-labelledby="converter-title">
        ...
        <VCard v-if="inspection" class="mt-3 pa-3" variant="outlined"><strong>{{ inspection.adapter?.name ?? "Unknown format" }}</strong><span class="text-caption ml-2">{{ inspection.bytes ?? 0 }} bytes</span><p class="text-body-2 mt-2">{{ inspection.message }}</p><p v-if="inspection.ambiguous" class="text-caption">Several adapters match these bytes. Choose the intended adapter or PDF operation deliberately.</p><p v-if="selectedAdapter" class="text-caption">Loss handling: {{ selectedAdapter.lossiness }}. The source remains untouched.</p><div class="mb-converter__targets"><VBtn v-for="option in targetOptions" :key="option" size="small" :variant="targetExtension === option ? 'flat' : 'outlined'" :color="targetExtension === option ? 'primary' : undefined" @click="targetExtension = option">{{ option.toUpperCase() }}</VBtn></div><VBtn class="mt-3" color="primary" :disabled="!targetPath || !selectedAdapter?.available" @click="convert">Convert to {{ targetExtension?.toUpperCase() ?? "target" }}</VBtn><p v-if="!selectedAdapter?.available" class="text-caption mt-2">This adapter is unavailable: {{ selectedAdapter?.unavailableReason }}</p><p v-else-if="targetPath" class="text-caption mt-2">Output: {{ targetPath }}</p></VCard>
        <VCard v-if="isPdfOperation" class="mt-3 pa-3" variant="outlined"><h2>PDF operation</h2><p class="text-caption">Choose the real operation, review inputs and outputs, then run the bundled offline PDF engine.</p><VBtn size="small" variant="tonal" :prepend-icon="mdiFolderOpenOutline" @click="pickPdfInput">Add PDF input</VBtn><VTextarea v-model="source" class="mt-2" label="PDF inputs, one path per line" rows="2"/><ConfigSearchField v-model="pdfOperationSearch" v-model:regex="pdfOperationRegex" v-model:flags="pdfOperationFlags" class="mt-2" label="Search PDF operations" :sample="pdfOperations.join('\n')"/><VSelect v-model="pdfOperation" class="mt-2" label="Operation" :items="visiblePdfOperations"/><VTextField v-model="pdfPages" label="Pages, one-based, comma or space separated" hint="Leave empty for every page" persistent-hint/><VSelect v-if="pdfOperation === 'rotate'" v-model="pdfRotation" label="Rotation" :items="[0, 90, 180, 270]"/><VTextField v-model="pdfOutput" label="Output PDF path" hint="Use a native output-folder browse above, or enter a filename" persistent-hint/><VTextarea v-if="pdfOperation === 'split'" v-model="pdfOutputs" label="Split output paths, one per line" rows="2"/><div v-if="pdfOperation === 'metadata'" class="mb-converter__metadata"><VTextField v-model="pdfTitle" label="Title"/><VTextField v-model="pdfAuthor" label="Author"/><VTextField v-model="pdfSubject" label="Subject"/><VTextField v-model="pdfKeywords" label="Keywords, comma separated"/><VTextField v-model="pdfCreator" label="Creator"/><VTextField v-model="pdfProducer" label="Producer"/></div><VCheckbox v-model="pdfOverwrite" label="Allow overwrite after super-confirmation"/><VTextField v-if="pdfOverwrite" v-model="pdfOverwriteConfirmation" label="Type I_UNDERSTAND_OVERWRITE"/><VBtn color="primary" class="mt-2" :disabled="pdfOperation !== 'inspect' && (!pdfOutput || (pdfOverwrite && pdfOverwriteConfirmation !== 'I_UNDERSTAND_OVERWRITE'))" @click="runPdf">Run PDF {{ pdfOperation }}</VBtn><p class="text-caption mt-2">Review: {{ source?.split(/\r?\n/).filter(Boolean).length ?? 0 }} input(s), {{ parsedPdfPages.length || 'all' }} selected page(s), output {{ pdfOutput || targetPath || 'not chosen' }}.</p></VCard>
