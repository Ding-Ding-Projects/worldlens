...
export interface McServerHost {
    ...
    readonly java?: {
        resolve(version: string): Promise<Answer<unknown>>;
        provision?(version: string): Promise<Answer<unknown>>;
...
export function createServerStore(options: ServerStoreOptions = {}): ServerStore {
    ...
    return {
            ...
            return value === null
                ? {
                      ...
                      failure: {
                          ...
                          message: "The Java discovery response was not a valid runtime report.",
                          detail: null,
                      },
                  }
                : { ok: true, value };
        ...
        onJavaProgress(listener): () => void {
            ...
            return host.java.onProgress((_serverId: string, event: unknown) => {
                if (isJavaProvisionProgress(event)) listener(event);
            });
        },
