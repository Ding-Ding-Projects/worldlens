...
async function startExport(overwrite = false, overwriteToken?: string): Promise<void> {
    ...
    const answer = await bridge.exportMap({ renderId: selectedRenderId.value, destination: destination.value.trim(), format: format.value, basePath: basePath.value.trim(), noJekyll: noJekyll.value, compression: compression.value, overwrite, ...(selectedMaps.value.length > 0 ? { maps: selectedMaps.value } : {}), ...(overwriteToken === undefined ? {} : { overwriteToken }), ...(format.value === "7z" ? { sevenZipOptions: { level: sevenZipLevel.value, threads: sevenZipThreads.value, solid: sevenZipSolid.value, dictionaryKb: sevenZipDictionaryKb.value } } : {}) });
