...
<template>
    <VCard class="mb-4 static-export-card" data-test="static-export-card">
        ...
        <VCardText>
            <p class="text-medium-emphasis mb-4">Build a self-contained static site, preview the selected scope, and validate every staged file before the result is reported.</p>
            ...
            <template v-else>
                ...
                <VAlert type="info" variant="tonal" class="mt-4" data-test="static-export-preview"><strong>Preview</strong><span class="ml-2">{{ selectedMaps.length || mapItems.length }} map scope entries · {{ estimatedFiles || "file count appears during inspection" }} files after inspection · no external CDN or tracking.</span></VAlert>
                <section v-if="state === 'running' || state === 'cancelled' || state === 'finished'" class="static-export-progress mt-4" aria-live="polite" data-test="static-export-progress"><div class="d-flex justify-space-between ga-3 flex-wrap"><strong>{{ phase }}</strong><span>{{ progress }}% · {{ completedFiles.toLocaleString() }} / {{ totalFiles.toLocaleString() }} files</span></div><VProgressLinear :model-value="progress" color="primary" height="10" rounded class="mt-2" /></section>
                <VAlert v-if="message !== null" type="error" variant="tonal" class="mt-4">{{ message }}</VAlert>
                <VAlert v-if="report !== null" type="success" variant="tonal" class="mt-4" data-test="static-export-result"><strong>Export created</strong><p class="mt-1">{{ report.fileCount.toLocaleString() }} files · {{ formatBytes(report.bytes) }} · {{ report.format.toUpperCase() }}</p><p class="mt-1">Manifest: <code>worldlens-export-manifest.json</code></p><p class="mt-1">The manifest records per-file SHA-256 checksums, map metadata, engine provenance, and omissions.</p></VAlert>
            </template>
        </VCardText>
