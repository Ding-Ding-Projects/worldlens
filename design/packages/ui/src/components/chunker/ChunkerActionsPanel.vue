...
async function refreshRepositories(): Promise<void> {
    ...
    else message.value = answer?.message ?? "Repository discovery is unavailable. Open GitHub accounts in Settings.";
}
...
<template>
    <section data-test="chunker-actions-panel">
        ...
        <section v-if="record" data-test="chunker-actions-progress">
            ...
            <ul><li v-for="job in record.jobs" :key="job.id">{{ job.name }}: {{ job.status }} · {{ job.conclusion ?? 'pending' }}</li></ul>
        ...
        <GhEntityPicker v-if="history.length" :model-value="record?.id" :items="history.map(entry => ({ title: `${entry.request.owner}/${entry.request.repo}: ${entry.state} ${entry.updatedAt}`, value: entry.id }))" data-test-base="chunker-actions-history" search-label="Search saved conversions" select-label="Resume saved conversion" selected-label="Selected conversion" empty-message="No saved conversions" no-match-message="No matching conversion" @update:model-value="id => record = history.find(entry => entry.id === id)" />
    </section>
</template>
