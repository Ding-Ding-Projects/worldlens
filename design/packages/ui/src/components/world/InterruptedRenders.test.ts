...
function fakeOffers(): ResumeOffers {
    return {
        ...
        active: ref([]),
        loading: ref(false),
        loadState: ref<"idle" | "loading" | "loaded" | "failed">("loaded"),
        recoveredCount: ref(OFFERS.length),
        skippedActiveCount: ref(0),
        activeState: ref<"known" | "unknown">("known"),
        lastAction: ref<"resumed" | "refused" | "dismissed" | null>(null),
        failure: ref(null),
        refusals: ref({}),
        busy: ref(null),
        available: true,
