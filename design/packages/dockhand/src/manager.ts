...
import { probeDocker, type DockerReport } from "./docker.js";

/**
 * Who owns a container, and therefore who is allowed to touch it.
 *
 * Every container this manager creates is labelled, and it will only ever list, stop or
 * remove containers carrying its own labels. Unlabelled containers, and containers labelled
 * by a different owner, are invisible to it - which is what makes it safe to point two
 * applications at the same Docker host.
 *
 * That safety is the whole reason this is a parameter rather than a constant. It used to be
 * hardcoded to the desktop application's namespace, which was correct while there was one
 * application. A second one constructed with the same namespace would inherit the first's
 * containers: it would list them, offer to stop them, and be right to, because by its own
 * labels they would be its own. The default below is exactly the value that was hardcoded,
 * so the desktop application's already-running containers stay visible to it.
 */
export interface ManagerIdentity {
    /** Reverse-DNS prefix for every label, without a trailing dot. */
    readonly labelNamespace: string;
    /** Prefix for the owner hash and for the volume names this manager will accept. */
    readonly ownerPrefix: string;
}

export const WORLDLENS_IDENTITY: ManagerIdentity = Object.freeze({
    labelNamespace: "com.worldlens",
    ownerPrefix: "worldlens",
});

/** The five labels, derived from an identity. */
export function managerLabels(identity: ManagerIdentity): {
    ...
    const prefix = identity.labelNamespace;
    return {
        hosting: `${prefix}.docker-hosting`,
        instance: `${prefix}.docker-instance`,
        name: `${prefix}.docker-name`,
        version: `${prefix}.docker-version`,
        owner: `${prefix}.docker-owner`,
        fingerprint: `${prefix}.fingerprint`,
    };
}

/**
 * Where a published port is reachable from.
 *
 * `loopback` is the default and stays the default: a port bound to 127.0.0.1 is reachable
 * from the machine running Docker and nowhere else, which is the right answer for something
 * a person opens on their own computer. `public` binds every interface, which is what a
 * deployment onto a server is *for* - and is why this exists at all, because the port
 * arguments used to be hardcoded to loopback with no way to say otherwise.
 */
export type PortBindMode = "loopback" | "public";

export interface PortMapping {
    readonly port: number;
    readonly bindMode?: PortBindMode;
}

/** How a container behaves after it exits or the host reboots. */
...
/** Deprecated aliases, kept so the desktop application's imports keep resolving. */
...
export interface CreateInstanceRequest {
    readonly id: string;
    readonly name: string;
    /** Always digest-pinned. A floating tag is refused; see `validateImage`. */
    readonly image: string;
    /**
     * Ports to publish.
     *
     * Plain numbers are still accepted and still mean loopback, so nothing that already
     * passed `[8100]` changed meaning. A {@link PortMapping} is how a caller asks for a port
     * that other machines can reach.
     */
    readonly ports?: readonly (number | PortMapping)[];
    readonly volumes?: readonly string[];
    readonly removeToken?: string;
    /**
     * Environment variables.
     *
     * Keys and values are both checked: a key that is not a shell-legal name, or a value
     * containing a newline, is refused rather than passed through. A newline in a value is
     * the interesting one - it cannot break out of `--env` as Docker passes it, but it can
     * corrupt anything downstream that reads the container's environment line by line.
     */
    readonly env?: readonly EnvironmentEntry[];
    /** Defaults to `unless-stopped`, which is what a deployed service almost always wants. */
    readonly restartPolicy?: RestartPolicy;
    /** Memory limit, in Docker's own notation (`512m`, `2g`). */
    readonly memory?: string;
    /** CPU limit, as a decimal number of cores (`1.5`). */
...
export interface DockerHostingManagerOptions {
    /**
     * Whose containers these are. Defaults to the desktop application's own namespace, so
     * that its already-running containers stay visible to it. A second application **must**
     * pass its own, or it will inherit the first's containers as though they were its own.
     */
...
/**
 * Read one `docker ps` row, or `null` when it is not ours.
 *
 * Takes the label set rather than reading module constants, because which labels count as
 * "ours" is now a property of the manager instance. Two managers on one host must be able
 * to look at the same row and disagree about whether it belongs to them.
 */
function managedContainer(
    value: DockerJsonContainer,
    owner: string,
    labelNames: ReturnType<typeof managerLabels>,
): ManagedInstance | null {
    const labels = typeof value.Labels === "object" && value.Labels !== null ? value.Labels as Record<string, unknown> : {};
    if (labels[labelNames.hosting] !== "true" || labels[labelNames.owner] !== owner) return null;
    const id = asString(labels[labelNames.instance]);
    const name = asString(labels[labelNames.name]) ?? asString(value.Names)?.replace(/^\//, "");
    ...
    return {
        ...
        fingerprint: asString(labels[labelNames.fingerprint]),
    };
}
...
export class DockerHostingManager {
    ...
    private readonly labels: ReturnType<typeof managerLabels>;
    ...
    public constructor(options: DockerHostingManagerOptions = {}) {
        ...
        this.identity = options.identity ?? WORLDLENS_IDENTITY;
        this.labels = managerLabels(this.identity);
        this.owner = `${this.identity.ownerPrefix}-${createHash("sha256").update(this.recordFile ?? "no-record-file").digest("hex").slice(0, 20)}`;
    ...
    public async list(): Promise<ManagerAnswer<readonly ManagedInstance[]>> {
        ...
        const output = await this.runner(this.docker, ["ps", "-a", "--filter", `label=${this.labels.hosting}=true`, "--format", "{{json .}}"]);
        ...
        const records = parseJsonLines(output.stdout).map((entry) => managedContainer(entry, this.owner, this.labels)).filter((entry): entry is ManagedInstance => entry !== null);
    ...
    public async snapshot(): Promise<ManagerAnswer<DockerHostingSnapshot>> {
        ...
        const [images, volumes] = await Promise.all([
            this.runner(this.docker, ["image", "ls", "--digests", "--filter", `label=${this.labels.owner}=${this.owner}`, "--format", "{{.Repository}}@{{.Digest}}"]),
            this.runner(this.docker, ["volume", "ls", "--filter", `label=${this.labels.owner}=${this.owner}`, "--format", "{{.Name}}"]),
        ]);
    ...
    public async create(request: CreateInstanceRequest): Promise<ManagerAnswer<ManagedInstance>> {
        ...
        const ready = await this.requireReady();
        if (!ready.ok) return ready;
        const ports = DockerHostingManager.normalisePorts(request.ports);
        ...
        for (const volume of request.volumes ?? []) if (!volume.includes(":")) {
            ...
            if (inspected.ok && !inspected.stdout.includes(`${this.labels.owner}=${this.owner}`)) return { ok: false, failure: failure("not-owned", `Named volume '${volume}' is not owned by this installation.`) };
            const created = inspected.ok ? inspected : await this.runner(this.docker, ["volume", "create", "--label", `${this.labels.hosting}=true`, "--label", `${this.labels.owner}=${this.owner}`, volume]);
        ...
        const args = ["create", "--label", `${this.labels.hosting}=true`, "--label", `${this.labels.owner}=${this.owner}`, "--label", `${this.labels.instance}=${request.id}`, "--label", `${this.labels.name}=${request.name}`, "--label", `${this.labels.version}=1`, "--label", `${this.labels.fingerprint}=${fingerprint}`, ...this.portArgs(ports), ...this.runtimeArgs(request), ...this.volumeArgs(request.volumes ?? []), request.image];
    ...
    private async inspectById(id: string): Promise<ManagerAnswer<ManagedInstance>> {
        const output = await this.runner(this.docker, ["ps", "-a", "--filter", `label=${this.labels.instance}=${id}`, "--format", "{{json .}}"]);
        ...
        }).map((entry) => managedContainer(entry, this.owner, this.labels)).filter((entry): entry is ManagedInstance => entry !== null && entry.id === id);
    ...
    private validateCreate(request: CreateInstanceRequest): ManagerAnswer<null> {
        ...
        for (const entry of request.env ?? []) {
            if (!ENV_KEY.test(entry.key))
                return {
                    ok: false,
                    failure: failure(
                        "invalid-request",
                        `"${entry.key}" is not a usable environment variable name.`,
                    ),
                };
            // A newline cannot break out of `--env` as Docker passes it, but it can corrupt
            // anything downstream that reads the container's environment a line at a time.
            if (/[\r\n]/.test(entry.value))
                return {
                    ok: false,
                    failure: failure(
                        "invalid-request",
                        `The value of "${entry.key}" contains a line break, which cannot travel safely in an environment variable.`,
                    ),
                };
        }
        if (
            request.restartPolicy !== undefined &&
            !["no", "on-failure", "unless-stopped", "always"].includes(request.restartPolicy)
        )
            return {
                ok: false,
                failure: failure("invalid-request", "That is not a Docker restart policy."),
            };
        if (request.memory !== undefined && !/^[0-9]+[bkmg]?$/i.test(request.memory))
            return {
                ok: false,
                failure: failure("invalid-request", "A memory limit looks like 512m or 2g."),
            };
        if (request.cpus !== undefined && !/^[0-9]+(\.[0-9]+)?$/.test(request.cpus))
            return {
                ok: false,
                failure: failure("invalid-request", "A CPU limit is a number of cores, like 1.5."),
            };
        return { ok: true, value: null };
    }

    /**
     * Publish a port, on the address the caller asked for.
     *
     * This used to be hardcoded to `127.0.0.1`, which is a fine default and was the only
     * option. A deployment onto a server exists to be reached from elsewhere, so a manager
     * that can only bind loopback cannot deploy one.
     */
    private portArgs(ports: readonly PortMapping[]): string[] {
        return ports.flatMap((mapping) =>
            (mapping.bindMode ?? "loopback") === "public"
                ? ["-p", `${String(mapping.port)}:${String(mapping.port)}`]
                : ["-p", `127.0.0.1:${String(mapping.port)}:${String(mapping.port)}`],
        );
    }

    /** Environment, restart policy and resource limits, each appended only when asked for. */
    private runtimeArgs(request: CreateInstanceRequest): string[] {
        const args: string[] = [];
        for (const entry of request.env ?? []) args.push("--env", `${entry.key}=${entry.value}`);
        args.push("--restart", request.restartPolicy ?? "unless-stopped");
        if (request.memory !== undefined) args.push("--memory", request.memory);
        if (request.cpus !== undefined) args.push("--cpus", request.cpus);
        return args;
    }

    /** Plain numbers still mean loopback, so an existing caller's meaning is unchanged. */
    private static normalisePorts(
        ports: readonly (number | PortMapping)[] | undefined,
    ): PortMapping[] {
    ...
    private validVolume(volume: unknown): volume is string {
        ...
        if (separator < 0) return new RegExp(`^${this.identity.ownerPrefix}-[a-z0-9][a-z0-9_.-]{0,120}$`).test(volume);
        return false;
    }
     ...
     *
     * Probes the address the caller actually asked for, which is not a detail. This used to
     * test `127.0.0.1` for every port whatever was requested, and the two answers genuinely
     * differ: something bound to `0.0.0.0:8100` does not answer on loopback in every
     * configuration, and something bound only to loopback leaves `0.0.0.0:8100` free. So a
     * loopback-only probe could report a public port free when it was taken, and the failure
     * would arrive later from Docker, phrased as a bind error rather than as a chosen port
     * being in use.
     */
    private async probePorts(ports: readonly PortMapping[]): Promise<ManagerFailure | null> {
        for (const mapping of ports) {
            const publicBind = (mapping.bindMode ?? "loopback") === "public";
            const host = publicBind ? "0.0.0.0" : "127.0.0.1";
            const busy = await new Promise<boolean>((resolve) => {
                const socket = createConnection({ host, port: mapping.port });
            ...
            if (busy)
                return failure(
                    "invalid-request",
                    `Port ${String(mapping.port)} is already in use on ${publicBind ? "this machine" : "loopback"}; choose another port.`,
                );
        }
        return null;
    }
