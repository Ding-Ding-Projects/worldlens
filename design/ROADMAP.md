...
| Banner pattern compatibility | Typed, ordered banner layers across supported Minecraft eras, with legacy/current aliases, opaque unknown identifiers, malformed-entry handling, and an exact acceptance boundary | **Pending.** Issue #89's public compatibility contract is documented in [`design/docs/banner-patterns.md`](docs/banner-patterns.md). Implementation, fixtures, round-trip proof, and real patterned-banner rendering evidence are still required; no completion claim is made here. |
