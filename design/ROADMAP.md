...
- [ ] **CI-render dispatch/poll/collect loop** (`sync.ts`). Waiting for a GitHub Actions run
      ...
      work is happening on GitHub's own runners - but the _local_ polling loop should stop
...
in application navigation with real saved-target and completed-map context. The records-only
pass intentionally ran no tests and took no captures; this entry is not a new verdict.
...
- [ ] Run focused contract coverage and capture the real packaged gate through the cheap headless
      route. Tests and captures are unrun in this records-only pass, so acceptance remains open.
...
| Banner pattern compatibility | Typed, ordered banner layers across supported Minecraft eras, with legacy/current aliases, opaque unknown identifiers, malformed-entry handling, and an exact acceptance boundary          | **Implementation, focused fixtures, and fixture-scoped oracle proof verified; packaged viewer proof remains open.** Typed data is in `47c3f8a5237f9f5f68c3aea63e92bc6cf13c4c1b`; focused executable fixture/round-trip proof is in `d14203e7e40a2ae4851b8bfe3476450609451570` (5/5). Malformed list elements now skip individually after one bounded diagnostic, later valid layers keep order, and diagnostic history caps at 32; reader-state failures still propagate. The manifest records a 64×64-block, 16-chunk fixture with exactly 3 banners and 10 ordered layers. `tools/oracle/out/patterned-banner/issue-89.json` accepts the expected TypeScript overlay delta over Java: exactly +60 vertices and +1 shared material group in the one hires tile, with every non-hires file matching. The packaged viewer returned the expected TypeScript hires SHA-256 and had live WebGL but stayed blank, so visible same-world read-back remains unverified. This records delta ran no new tests, builds, packaged interaction, or captures. See [`design/docs/banner-patterns.md`](docs/banner-patterns.md) and the 2026-08-19 handoff entry.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- Wave 1 completed **256/256** shards.
- Wave 2 completed **7/105** shards; **98** were cancelled in flight.
- The two-wave **merge was not reached**.

This is the complete observed dispatch record. It does not prove a final map, lowres rebuild,
merged metadata, public result, or hosted-runner disk boundary. The next run must reach the real
merge and retain disk, integrity, ordering, resumability, cleanup, and publication receipts.
