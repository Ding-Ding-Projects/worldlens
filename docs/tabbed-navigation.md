...
The documentation site's compact horizontal strip puts a three-dot button on every tab, including
pinned tabs. Tapping it opens the same searchable context menu as right-click, with pinning,
movement, grouping, closing, bulk-close, and appearance actions intact. The button names both the
action and its tab to assistive technology, uses the shared 44-pixel minimum touch target, and does
not activate the tab merely because its menu was opened. It stays out of the roving tab sequence;
keyboard users open that menu from the focused tab with the platform context-menu key.
