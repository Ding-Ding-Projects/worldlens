...
a grown-up is through it, hands control back to the real Adult Mode shell rather than mounting a
settings surface of its own - see "The grown-up gate" below for exactly what that means. Activation,
everywhere, still goes through `shellNavigation.ts`'s `activateTarget`, exactly as a click on the
adult Home's own cards does.
...
- Passing the gate does not reveal a settings surface by itself - it emits one event,
  `switchToAdult`, that `KidShell.vue` answers by setting `kid.enabled` to `false`. The whole kid
  shell unmounts and the ordinary Adult Mode shell (the tab strip, the map, everything) takes its
  place, exactly as if Kid Mode had never been on. From there, opening Settings the normal Adult
  Mode way surfaces a **Kid Mode and Adult Mode** row (`KidModeRow.vue`): a two-option choice
  between the two named modes, not a bare on/off checkbox, because "off" is not the name of a mode
  and a grown-up searching for "adult" needs real matching text to find. Picking Kid Mode there
  puts a child straight back where they left off, because nothing about their progress or their
  preferences lived inside the shell that was swapped out.

Both the "Kid Mode and Adult Mode" row and the shared restricted mode's own row are ordinary rows
...
computes or passes that prop: `App.vue` only ever builds `kidRenderRows` (the running-render rows)
and `kidProfiles` (the map and server list) for `<KidShell>`, and no equivalent backup/CI feed
exists for the adult Home either. So the "what is happening right now" panel shows running renders
and otherwise the quiet state today; the backup/CI half of it is a typed prop waiting for a real
source, not a wired feature. Pressing a land opens `KidCataloguePage.vue`, which groups that catalogue's
features exactly as they are grouped everywhere else and offers the same `filterCatalogues`/
`createCatalogueMatcher` search, with `ConfigRegexBuilder` behind the same anchored `.*` toggle
every other search bar in this application already carries. Nothing here keeps a list of its own;
everything is what the catalogue already says, laid out bigger.
...
the shipped feature name, as `"{kid label} - {shipped name}"` - a plain hyphen, not an em dash: this
project spells em dashes as ordinary words everywhere else its own copy is checked for one, and a
string built at runtime here is no exception. A screen reader, the tab finder, the
...
- **Nothing has been set up.** This is the state of a fresh install, because nobody has visited the
  ...
  exactly that - `credentialConfigured` is false - and lets the grown-up straight through with no
  prompt at all, and says so in its own copy rather than pretending a credential exists. This is
  also, in practice, how the first credential ever gets set: a grown-up walks straight through the
  very first time, finds the shared restricted mode's own row inside Adult Mode's settings, and
  sets one there if they want the gate to actually ask next time.
- **A credential exists.** Once one has been configured - by this route or from anywhere else in
  the application, because it is one shared record - the gate asks for it, clears the field on
  every attempt, and reports a wrong entry as "That code did not match. Kid Mode stays on." with no
  further detail and no escalation.

Either way, passing the gate is a single event rather than a settings surface of its own:
`KidGrownUpGate.vue` emits `switchToAdult`, and `KidShell.vue` answers it by setting `kid.enabled`
to `false` - nothing more. That flag is the one thing every kid-mode component already watches, so
the whole kid shell unmounts on its own and the ordinary Adult Mode shell - its own tab strip, its
own Settings, its own options editor, its own problems panel - takes its place, exactly as if Kid
Mode had never been on. An earlier draft of the gate revealed `AppSettings.vue` and the options
editor inline, still inside the kid-mode shell; that version needed a prop `KidShell.vue` never
actually passed down, so it could never have rendered correctly, and this is the simpler, correct
replacement for it. A grown-up who came here to fix a specific problem reaches that problem's own
remedy the same way any Adult Mode user does, once Adult Mode is showing - the gate itself shows
nothing beyond its own heading, blurb, field and honesty line.
...
and the XP bar in `KidShell.vue`'s status header both read straight from it. `award(id)` is
designed to be called only from the completion event of a real action - the eight stickers are
bound to rendering a first map, finding a world, adjusting render speed, dropping a marker,
backing up, publishing, running the automatic repair, and local version history - and it is a
deliberate no-op for a sticker already won, so nothing here can double-award or, more importantly,
congratulate a child for something the application did not actually do. `KidShell.vue` exposes
`award()` for exactly that purpose, the same way it exposes `ensureJob()` and `revealJob()` for
`App.vue` to drive. As of these sixteen files, though, `App.vue` calls `ensureJob()` and
`revealJob()` on that ref but never calls `award()`: no completion handler anywhere outside `kid/`
passes a sticker id to it yet, so while the ledger, the sticker book and the celebration are all
real and correctly wired to *each other*, nothing in ordinary play currently triggers the first
link in that chain - a child cannot yet earn XP or a sticker by using the app. `kidMode.contract.test.ts`
still checks every one of the eight stickers names a feature that genuinely exists in the
catalogue, which is a guarantee about what an award *would* mean if one were ever made, not a
claim that one currently is. `KidStickerBook.vue` shows every sticker, won or not, plainly
...
shipped) spoken narrator is specified to follow - and turning it on now does something audible:
`KidCelebration.vue` plays a short, synthesised two-note chime through the Web Audio API whenever a
celebration fires with `kid.sound` on. Nothing is bundled or fetched for it - there is no audio
asset in this checkout and no network request - so a synthesised tone is the honest way to keep the
preference true rather than aspirational. The call is wrapped defensively: a refused or unavailable
`AudioContext` (private browsing, no user gesture yet, the test environment) means silence rather
than a broken celebration. There is no separate "reduced sound" or "quiet hours" setting anywhere in
this checkout for the chime to also consult; if one is added later, it belongs beside `kid.sound` in
`kidMode.ts`.
...
root (`KID_SHELL_VARS`) rather than new tokens: rounded corners from 16px to a full pill, and a
64px minimum target - the adult shell's floor is 48px, and Kid Mode only ever raises it. The font
stack is Roboto, not a distinct rounded display face: `KID_SHELL_VARS`'s own comment records that
'Baloo 2' was never actually bundled in this checkout - no `@fontsource/baloo-2` dependency, no
`@font-face` rule, no local font file - so it would have silently fallen back to Verdana on every
machine that does not happen to have it installed, which is effectively all of them. Kid Mode
instead leans on the Roboto weights `main.ts` already bundles for the rest of the application, and
lets its own generous `font-weight: 800` rules throughout `kid/*.vue` do the "big and friendly"
work a distinct face would otherwise carry.
...
heading and its credential label among them - carries one string per language with no level. The
gate's own honesty statement (`kid.gate.honesty`) is itself `VOICED` rather than fixed: its wording
softens across the five funny levels, from a plain "this is a user-experience lock, not a security
lock" at level one to "just a friendly speed bump" at level five, while `KID_FACTS` pins the phrases
"not a security lock" and "Shared restricted mode" so no funny level can rewrite the honesty out of
it. Registered the same way every other surface's catalogue is, Kid Mode's strings reach all three
...
`App.vue` remembers whichever adult theme (dark, light, contrast, or follow-the-system) was showing
at the instant Kid Mode most recently turned on, under its own `bluemap-kid-remembered-adult-theme`
key, and switches Vuetify straight to `kid` without touching Adult Mode's own `bluemap-theme`
record - the one [Display and ease of use](./display-and-ease-of-use.md) describes - while Kid Mode
stays on. Turning Kid Mode off restores exactly that remembered theme, and *that* write does go
through the same `changeTheme()` every real theme control uses, so `bluemap-theme` ends up holding
it too. A machine that has never chosen a theme at all does not default to "follow the system" here:
`themeSetting.ts`'s own `FRESH_INSTALL_THEME` is `dark`, so that is what a fresh install remembers
and restores, exactly as it is what Adult Mode would have opened to on its own.
...
- **A grown-up's credential attempt does not match.** The field clears, a plain "That code did not
  match. Kid Mode stays on." is shown, and nothing about the attempt is recorded, escalated or rate
  limited by Kid Mode's own gate.
- **A completion event never calls `award()`.** Nothing breaks - the sticker book still opens, still
  shows every sticker as "Not yet" - but no XP or sticker is earned either, because nothing outside
  `kid/` currently passes a sticker id to `KidShell.vue`'s exposed `award()`. See the note above.
...
- 過咗閘口，唔會即刻見到一個設定介面——閘口淨係發一個事件 `switchToAdult`，`KidShell.vue` 收到就將 `kid.enabled` 設做 `false`。成個 kid shell 就自己 unmount，換上原本嗰個 Adult Mode shell（tab strip、地圖，全部嘢）。之後用返 Adult Mode 平時嗰種方法打開 Settings，先會見到一行 **Kid Mode and Adult Mode**（`KidModeRow.vue`）：一個喺 Kid Mode 同 Adult Mode 兩個具名模式之間揀嘅選項，唔係一粒開／關剔掣，因為「off」唔係一個模式嘅名，大人搵「adult」都要有真係搵得中嘅字。喺嗰度揀返 Kid Mode，小朋友就即刻返去佢離開嗰陣嘅樣，因為佢啲進度同偏好從來冇存喺俾換走嗰個 shell 入面。

「Kid Mode and Adult Mode」呢一行同共用嘅 restricted mode 自己嗰行,一樣係普通設定行,所以都用同一個方法搵得到：喺嗰個介面自己嘅搜尋欄,同喺 command palette 度,兩個名都搵得到。

#### 五個地方，同畫佢哋嗰個一模一樣嘅目錄

`KidHome.vue` 就係 Kid Mode 嘅 Home：一張 project editor 嘅 hero card（同成人 Home 個 hero 解算出嚟嗰個 `findFeature("make.finding-a-world.the-project-editor")` 一模一樣）、五粒掣代表五個目錄，做成五個「地方」（`Make a map`、`Your maps`、`Show people`、`Keep it safe`、`Buttons & help`）、一格顯示 app 而家做緊乜嘅面板，同一格列住呢部機識嘅地圖同伺服器。`KidHome.vue` 有個選填嘅 `activity` prop，型別係為 backup 同 CI 嗰行度身訂造，擺喺佢原本已經有嘅 render 行隔籬；但而家冇任何嘢會計出嚟或者傳呢個 prop：`App.vue` 淨係為 `<KidShell>` 砌咗 `kidRenderRows`（行緊嗰啲 render）同 `kidProfiles`（地圖同伺服器清單），成人 Home 都冇一個對應嘅 backup/CI feed。所以「而家做緊乜」呢個面板而家淨係會顯示緊行緊嘅 render，冇就顯示靜英英嗰句；backup/CI 嗰半截係一個等緊真正資料源嘅型別 prop，而唔係一個已經接埋線嘅功能。撳一個地方就打開 `KidCataloguePage.vue`，佢會按嗰個目錄本身嘅分組去分，用返同一個 `filterCatalogues`/`createCatalogueMatcher` 搜尋，`ConfigRegexBuilder` 就喺本應用程式其他每一條搜尋欄都用嘅嗰個 anchored `.*` 掣後面。呢度冇任何嘢自己維護一張清單；一切都係目錄講咩就係咩，淨係畫大啲。
...
`kidAccessibleName()` 就係令呢個安全到可以喺同一個 build 度畀成人用嘅嗰部分：無論標籤風格點揀，每一個掛咗 kid label 嘅控制項嘅 accessible name 都仍然帶住出貨嗰個功能名，格式係 `"{kid 標籤} - {出貨名}"`——一個普通嘅連字號，唔係破折號：呢個專案喺自己其他每處畀人查過嘅文案都係將破折號寫做普通字，喺呢度即場砌嘅字串都唔例外。所以螢幕閱讀器、分頁搜尋器、command palette 自己嗰個搜尋，同呢個專案 harness 影嘅每一張截圖，都仍然可以用文件用緊嗰個名認得返個功能——就算螢幕上寫住嘅係「Robot helpers」都好。`kidMode.contract.test.ts` 會喺三種標籤風格、全部八十四個功能都逐一查證，唔止查預設嗰種。
...
- **仲未設定過任何嘢。** 呢個就係一個全新安裝嘅狀態，因為冇人去過共用 restricted mode 自己嗰項設定畀佢一個 PIN、密碼或者 passkey。閘口讀到嘅正正就係咁——`credentialConfigured` 係 false——所以會讓大人直落過去，乜都唔問，而且會喺自己嘅文案度講明,唔會扮成好似有憑證噉。喺實際情況入面，呢個亦都係第一個憑證究竟點樣設定出嚟嘅方法：大人第一次一定係咁直落過去,喺 Adult Mode 嘅設定入面搵到共用 restricted mode 自己嗰行,如果想閘口下次真係會問就喺嗰度設定一個。
- **已經有憑證。** 一旦設定咗——由呢條路，定係由呢個應用程式任何其他地方，都得，因為佢係同一份共用紀錄——閘口就會問，每次輸入之後都清空個欄位，輸錯就報「That code did not match. Kid Mode stays on.」（個碼啱唔到，Kid Mode 繼續開住），唔會講多，亦唔會升級。

無論邊種情況，過咗閘都淨係一個事件，而唔係佢自己一個設定介面：`KidGrownUpGate.vue` 發個 `switchToAdult`，`KidShell.vue` 收到就將 `kid.enabled` 設做 `false`——就係咁多。呢支旗標係每一個 kid-mode component 早就睇緊嗰個，所以成個 kid shell 自己就會 unmount，換上原本嗰個 Adult Mode shell——佢自己嘅 tab strip、自己嘅 Settings、自己嘅選項編輯器、自己嘅問題面板——就好似 Kid Mode 從來冇開過噉。閘口舊版本試過將 `AppSettings.vue` 同選項編輯器直接掛喺 kid-mode shell 入面，仲留喺原地；嗰個版本需要一個 `KidShell.vue` 從來冇傳落去嘅 prop，所以根本冇可能 render 得啱，而而家呢個先係啱、亦簡單得多嘅做法。一個因為有問題要解決先嚟到呢度嘅大人，一旦 Adult Mode 顯示出嚟，就同任何 Adult Mode 用戶一樣搵得返嗰個問題自己嘅補救掣——閘口本身除咗自己個標題、文案、輸入欄同坦白句，乜都唔會顯示。

#### XP、等級同貼紙，每一個都繫住真正發生過嘅嘢

`useKidProgress.ts` 用一本只加不減嘅紀錄簿記低攞咗邊啲貼紙、同每個貼紙帶嚟嘅 XP，本地儲喺 `bluemap-kid-progress`。等級係 `floor(xp / 500) + 1`；`KidShell.vue` 狀態列度嗰個等級讀數同 XP 條都直接讀呢度。`award(id)` 嘅設計係淨係由一個真正動作嘅完成事件先觸發——八個貼紙分別繫住 render 第一張地圖、搵到一個 world、調校 render 速度、擺一個 pin、備份、發佈、跑自動修復，同本地版本歷史——而且對一個已經攞過嘅貼紙係刻意乜都唔做，所以呢度冇可能重複頒獎，更加唔可能為應用程式冇做過嘅嘢恭喜緊小朋友。`KidShell.vue` 公開咗 `award()` 就係為咗俾人咁樣用，同佢公開 `ensureJob()`、`revealJob()` 俾 `App.vue` 去揸一樣。不過截至呢十六個檔案，`App.vue` 有叫呢個 ref 嘅 `ensureJob()` 同 `revealJob()`，但從來冇叫過 `award()`：`kid/` 以外冇任何一個完成事件嘅 handler 傳個貼紙 id 入嚟，所以雖然紀錄簿、貼紙簿同慶祝三樣嘢之間接得完全啱，但依家用app都唔會觸發到呢條鏈最頭嗰一環——小朋友暫時用呢個 app 都攞唔到 XP 或者貼紙。`kidMode.contract.test.ts` 仍然會查證全部八個貼紙每一個都指名一個目錄入面真係存在嘅功能，但呢個係「如果真係頒咗獎，個獎會指緊乜」嘅保證，唔係話而家真係已經頒緊獎。`KidStickerBook.vue` 會將每一個貼紙都展示出嚟，無論攞咗未攞都清清楚楚標明——冇任何嘢係收埋或者吊人胃口——撳一個就打開佢係由邊個功能攞返嚟嘅，所以本簿都變咗做多一條入去目錄嘅路，而唔係一個死嘅獎座架。
...
「Play a sound with a celebration」呢個剔掣（`kid.sound`）預設係關嘅，同「慶祝」呢個設定本身各自獨立持久化。佢自己嘅型別註解講得好白——喺減少聲音或者靜音時段生效嗰陣就應該靜聲，同呢個應用程式自己（仲未出貨嘅）語音旁述所訂明嘅習慣一樣——而而家剔開呢粒掣係真係有嘢聽嘅：`KidCelebration.vue` 一慶祝而且 `kid.sound` 開住，就會用 Web Audio API 播一段自己合成嘅兩個音嘅短叮聲。呢度冇 bundle 任何音效檔案，亦都冇網絡請求——冇音效檔案喺呢個 checkout 入面，一個都冇——所以用合成嘅音先係令呢個偏好講到做到、而唔係得個講字嘅老實做法。呼叫本身包咗防守性代碼：`AudioContext` 攞唔到或者俾人拒絕（私隱瀏覽、仲未有用戶手勢、測試環境）就靜聲，而唔係整壞成個慶祝。呢個 checkout 入面冇一個獨立嘅「減少聲音」或者「靜音時段」設定俾呢個叮聲一齊查；如果之後加咗一個，就應該擺喺 `kidMode.ts` 入面 `kid.sound` 隔籬。

#### 顏色同形狀，借返嚟嘅 role

`kidTheme.ts` 嘅 `KID_SCHEME` 答嘅係 `colorRoles.ts` 已經宣告嗰批 Material Design 3 role 名——`primary`、`primary-container`、`surface-container-high` 等等——而唔係發明一個新 token。佢保留住產品本來嗰隻 seed 藍色，將啲 surface 移去淺色、高彩度嘅處理，用返太陽黃做 `tertiary`。`kidMode.contract.test.ts` 會斷言 kid scheme 嘅 key 集合，排序之後同 dark scheme 一模一樣，所以呢度唔會靜靜雞加多一個第二個 theme 唔使答嘅 role。形狀同密度就係 shell root 上面嘅 CSS 自訂屬性（`KID_SHELL_VARS`），唔係新 token：圓角由 16px 一路去到全圓，同一個 64px 嘅最低目標——成人 shell 嘅下限係 48px，Kid Mode 淨係會加，唔會減。字型堆疊用嘅係 Roboto，唔係一套獨立嘅圓體展示字：`KID_SHELL_VARS` 自己個註解記低咗「Baloo 2」喺呢個 checkout 從來冇真正 bundle 過——冇 `@fontsource/baloo-2` 呢個依賴、冇 `@font-face` 規則、冇本地字型檔——所以但凡機度冇裝呢隻字（實際上幾乎每部機都係咁），佢就會靜靜雞跌返去用 Verdana。Kid Mode 而家改用 `main.ts` 已經 bundle 咗俾成個應用程式用嘅 Roboto 幾隻字重，再靠 `kid/*.vue` 入面自己嗰啲慷慨嘅 `font-weight: 800` 規則,去做返本來要靠一套獨立展示字先做到嘅「大大隻、友善友善」效果。

#### Kid Mode 自己嘅文案，同佢刻意避開嘅上游陷阱

`kidCopy.ts` 用嘅係同其他每一個介面文案檔一樣嘅 `VOICED`/`FIXED` 形狀，[Language modes and funny levels](./language-and-tone.md) 有完整講解：每一個 `VOICED` entry 帶五句英文加五句廣東話，一 level 一句；每一個 `FIXED` entry——包括大人閘口個標題同佢個憑證標籤——每種語言一句，冇 level 之分。閘口自己嗰句坦白（`kid.gate.honesty`）反而係 `VOICED`，唔係 fixed：文案會跟住五個 funny level 變得越嚟越輕鬆，由 level 一嗰句樸樸實實「呢個係使用體驗鎖，唔係保安鎖」，到 level 五「淨係一條得意嘅擋路帶啫」；但 `KID_FACTS` 釘死咗「唔係保安鎖」同「Shared restricted mode」呢兩句字眼，令邊個 funny level 都改唔走呢句坦白嘅重點。用返同其他介面目錄一樣嘅方式登記，Kid Mode 嘅文字就掂到全部三個語言模式同兩條逐語言 funny level 滑桿，冇一個 component 使改。
...
換 shell 同時亦係換 theme，而且係刻意噉做嘅。Kid Mode 嘅顏色係註冊做佢自己一個具名嘅 Vuetify theme（`kid`），而唔係塗喺原本活躍緊嗰個成人 theme 上面，所以閂咗 Kid Mode 唔會令個 app 睇落去仲係 Kid Mode、淨係冇咗條 rail。`App.vue` 會記低 Kid Mode 最近一次開嗰一刻，成人 theme（dark、light、contrast，定係跟系統）當時係邊個，記喺佢自己嗰個 `bluemap-kid-remembered-adult-theme` key 底下，然後直接將 Vuetify 切去 `kid`，Kid Mode 開住嗰段時間唔會掂 [Display and ease of use](./display-and-ease-of-use.md) 講嗰個 Adult Mode 自己嘅 `bluemap-theme` 紀錄。閂咗 Kid Mode 就會將記低嗰個 theme 復原，而呢一次寫入係真真正正經過同一個 `changeTheme()`（同任何一個真正嘅 theme 掣一樣），所以 `bluemap-theme` 最終都會俾呢個值寫返入去。一部從未揀過 theme 嘅機喺呢度唔會預設「跟系統」：`themeSetting.ts` 自己嗰個 `FRESH_INSTALL_THEME` 係 `dark`，所以一個全新安裝記低同復原嘅都係 `dark`，同 Adult Mode 自己一開機會用嗰個一模一樣。
...
- **一個大人打嘅憑證啱唔到。** 個欄位會清空，出返一句白話「That code did not match. Kid Mode stays on.」（個碼啱唔到，Kid Mode 繼續開住），而呢次嘗試 Kid Mode 自己個閘口唔會記錄、唔會升級，亦都唔會限速。
- **冇任何完成事件叫過 `award()`。** 唔會壞——貼紙簿照樣打得開，每個貼紙照樣顯示「未攞到」——但都攞唔到任何 XP 或者貼紙，因為 `kid/` 以外冇任何嘢而家會傳個貼紙 id 入 `KidShell.vue` 公開嗰個 `award()`。見返上面嗰段。
...
`kidMode.contract.test.ts` 由源頭登記表本身去斷言：八十四個目錄功能、十八個 job、十九個 settings section 同五個目錄，每一個都有 kid label，數字係推導出嚟而唔係寫死喺測試度；accessible name 喺三種標籤風格、全部功能都仍然帶住出貨嗰個功能名；kid 色彩配置排序之後同 dark scheme 一模一樣，一個 role 都唔多唔少；每一個貼紙都指名一個目錄入面真係存在嘅功能。
