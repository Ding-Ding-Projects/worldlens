...
| The selected account is signed in but inactive when a release operation begins                       | The app switches to it with the exact host and login for that operation, re-reads the account inventory, verifies the effective API identity, then restores the account that was active beforehand; a restore refusal is reported. |
