...
`tools/oracle/fixtures/patternedBannerWorld.mjs`. It writes three banner block entities into
one valid Anvil chunk: a legacy `Patterns` banner with numeric dye ids, a current
`patterns` banner with namespaced identifiers and color names, and a modern wall-banner
record. The three banners carry ten ordered layers in total. The generated
