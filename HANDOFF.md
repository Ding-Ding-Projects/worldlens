...
| Chunker on GitHub Actions, 1 GB | verified | `chunk-world.yml` from `a924d330` in `builders-home/wl-render-1gb-20260904`, run 33949082305 success in 10 min 52 s; `converted-world.zip` matched its SHA256SUMS and holds `level.dat` plus a 178-file LevelDB, 372,080,983 bytes, within 0.005 % of the local conversion |
| Chunker round trip, 10 GB, local | verified (`clean`) | Both legs through bundled Chunker 1.19.1 (3.75 GB Bedrock, 10.0 GB back); the harness comparison segfaulted once (fixed in `0ffcf0af`), then `--compare-only` on the retained legs: 2,436,647 / 2,436,647 chunks, all matched, 0 undocumented, 0 missing, 0 extra, only DataVersion and heightmap changes; `docs/evidence/roundtrip-10gb-r3-evidence.json` |
| Shipped v1.0.2008 templates, tiny world | verified | `builders-home/wl-render-10gb-20260904` run 33949562730 on the packaged `render-world.yml` bytes (toolchain pin `40680d79`): success in 9 min 16 s, `rendered-map` 2,238,322 bytes |
| Chunker screen exposes every dispatch input | verified | `28294f9d`: eight of the ten `chunk-world.yml` inputs had no control; all ten now wired and validated on both sides, `scripts/check-chunker-ui-inventory.mjs` plus `workflowUiInventory.test.ts` fail when one is missing. Debt: the chunker copy surface is not yet in the copy coverage gate |
| App fetches multi-group renders | verified in the built app | `545b74e3`: `collectRenderedMap` assembles `map-lowres` + every `partial-hires-N` with per-part digests and Range resume; a gap refuses as `map-parts-incomplete`. Six tests watched red then green. Then proven from the packaged v1.0.2012 build (extracted `.nupkg`, isolated profile, hidden desktop, CDP): `resumeCiRender` on a seeded dispatched record for run 33941015721 fetched `map-lowres` + `partial-hires-0..5` (17,533,159,711 bytes, every artifact byte-exact to GitHub), 30.6 min, app summary `outcome: rendered, verified: true`, 607,585 tiles in `tiles/0` with every entry of every part present and no duplicates. Gap: no UI path to fetch a run this device did not dispatch (the record had to be seeded), a pig is adding one |
| Fetch a render made elsewhere | verified in the built app | `ef213554`: Cloud Render card lists a repository's completed `render-world.yml` runs (run name now `Render <map> (<dimension>)`, template version 6) and collects one this device never dispatched; `#finishRecordedRun` now requires only the run id. 12 new tests, one watched red then green; 57 focused files green, tsc/vue-tsc 0, drift/lint/parity clean. Driven in the packaged v1.0.2016 app: list, select and fetch all worked, 17.3 GB fetched and verified, then registration refused (`artifact-not-a-map`) because attach assumed the local project's map id. `450fe89d` derives the map id from the run title and the artifact contents (`artifact-multiple-maps` refusal names the candidates, editable `Register as map` field), 7 new tests, one watched red then green. Re-driven in packaged v1.0.2018 against run 33941015721: the map-id field stayed empty (that repository's runs still carry the pre-v6 title "Render world"), collect fell back to the artifact's own `fixture_10gb`, and the Renders list shows `fixture_10gb · GitHub's runners · Finished`; `sync.json` stage rendered, 607,585 tiles under `web/maps/fixture_10gb` |
...
desktop UI into task-owned `builders-home` repositories and GitHub Pages. AWS is excluded.
The prior successful Actions run used a 64 MB world and skipped Pages, so it does not prove
the requested 1 GB or 10 GB rows. The recorded large local fixtures have not yet been located.

The headless driver now omits renderer authentication queries from diagnostics and capture
receipts and checks the complete DevTools target inventory before attachment. Its three
focused privacy tests pass. A new project-local diagnosis skill records the working launch,
window selection and URL-redaction method. This is tooling proof, not large-world acceptance.
Generator UI wiring, full Chunker configuration and destination dispatch, server creation,
current packaging, and the full runtime matrix remain in progress.

The supported `build.bat /s` route exposed a portable-tool bootstrap failure: the shell could
download Node but could not resolve `Get-FileHash`. Acquisition now hashes and extracts with
framework APIs and has a 120-second download deadline. Eleven focused build/bootstrap tests
pass, including real ZIP extraction, refusal before extraction on a wrong digest, and a
fixture where the two archive cmdlets are unavailable. The complete build is not yet verified.

## 2026-09-04: destination smoke test at 1 GB and 10 GB, and a double-install that hung a button

**Two synthetic worlds, generated by the committed `worldgen` and measured rather than estimated.**

| World | Bytes | Regions | Generated in |
| --- | --- | --- | --- |
| 1 GB | 1,026,097,718 | - | 138 s |
| 10 GB | 10,271,638,069 | 2,500 | 1,659 s (27.6 min) |

**The 1 GB rendered clean end to end: 3,571 s (59.5 min), exit 0.** 62,354 tiles across four LOD
levels, 2.49 GB of output. Validated with the shipped tool rather than by eye:

```
static-host --check  →  servable: true, 62,500 files, 2,486,253,550 bytes
                        missing: [], oversized: [], missing assets: []
```

That is also the first time `prepareStaticHost` has been run against a real multi-gigabyte map
rather than a fixture.

**The 10 GB render was stopped at 6.75%, deliberately.** It had produced 43,349 tiles and was
reporting a 10-hour ETA, which is consistent with the 1 GB run and gives a usable figure: roughly
one hour per gigabyte of world on this machine, single process. Nothing about it failed. Anyone
resuming it can point `render10/config/maps/overworld.conf` at the world again; BlueMap picks up
where it left off.
...
workloads that have nothing to do with this.

**AWS: excluded at the user's instruction.**

### What the sweep found

The suite went from 11 failures to **13,195 passing, zero failures**, and the typecheck baselines
from 39 and 10 to zero. Three findings are worth carrying:

**Double-clicking Install Java ran the whole download twice and hung the button.** `provisionJava`
waited for whatever held the Java lock without asking which kind it was. Waiting for a *check* and
then provisioning is the point of that code; waiting for a *provision* and then provisioning is a
second install of something just installed - and the second promise never settled, so the spinner
stayed up for good.

**An interface was declared twice in one file**, so TypeScript merged the two into a type that
disagreed with itself about whether a paused backup's kind can be null.

**Three fixtures set `canPause` twice in one object literal**, where JavaScript silently takes the
second - so all three ran as `true` while appearing to offer a choice.

廣東話同步：兩個世界都係用 committed worldgen 整出嚟，數字係度出嚟唔係估。1 GB 行足 59.5 分鐘
...
wrong-version, wrong-digest, absent-JAR, malformed identity, oversized, and symlinked inputs
fail before electron-builder starts. Version, source commit, workflow run ID, and run attempt
are validated before the downloaded artifact is touched. Containment, symlink identity, size,
and JAR bounds are checked before the bounded streaming digest pass.
...
cover the success path, refusal cases, and deliberate mutations of the canonical identity call
and digest comparison. Local app build, typecheck, package-contract tests, actionlint structural
validation, diff checks, and the public vocabulary scan remain the verification record. The
