...
| App fetches multi-group renders | verified in tests | `545b74e3`: `collectRenderedMap` assembles `map-lowres` + every `partial-hires-N` with per-part digests and Range resume; a gap refuses as `map-parts-incomplete`. Six tests watched red then green; not yet exercised against a live multi-group run from the app |
