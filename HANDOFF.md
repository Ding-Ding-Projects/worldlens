...
| Fetch a render made elsewhere | verified in tests | `ef213554`: Cloud Render card lists a repository's completed `render-world.yml` runs (run name now `Render <map> (<dimension>)`, template version 6) and collects one this device never dispatched; `#finishRecordedRun` now requires only the run id. 12 new tests, one watched red then green; 57 focused files green, tsc/vue-tsc 0, drift/lint/parity clean. Not yet driven in the built app |
