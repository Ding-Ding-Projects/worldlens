...
| Docker render of the 1 GB world | verified | Released image `ghcr.io/ding-ding-projects/worldlens-cli:1.0.2006` with `-c /data/config -r`, config generated by the image itself; ran paused/unpaused around the Chunker legs, finished 17:xx UTC with exit 0; `web/maps/overworld/tiles/0` holds 60,426 tile files, `settings.json` and `textures.json.gz` present, 2.4 GB; the GitHub Actions render of the same world produced 60,759 hires tiles, the difference is lowres/pyramid layout between the two BlueMap builds, not missing area |
| Left rail More menu and label clipping (#180) | verified, released v1.0.2031 | the overflow menu bound a second click on its own activator and closed itself in the same press; compact rows now wrap a long label instead of ellipsizing; packaged captures in `packaged-release-issues-180-183` |
| Convert screen folder buttons disabled (#181) | verified, released v1.0.2030 | the world list read its picker from an injection only three screens provide; falls back to the dialog bridge, or states its reason |
| Chunker not bundled, Get Chunker no-op (#182) | verified, released v1.0.2035 | the jar shipped all along; the resolver never looked in resources and the screen never listened for the picker's fix event; bundled-first with a pinned digest, afterPack assertion, verified download fallback; jar proven inside the nupkg |
| Chunker destination ignored, label overlap (#183) | verified, released v1.0.2034 | the route picker listened on each radio, which owns no model inside a radio group; handler moved to the group, route persisted, 16 px token gap |
| Adversarial review of the day's commits | 13 of 16 fixes released v1.0.2038 | six-lens finders, three refuters per finding, one fixer per confirmed finding with a red-first test, independent verification; three duplicate fixes retained unmerged on their branches (df239244, f27ac3bd, 6ff5b62d) |
| Capture-evidence debt | five stale groups | the four pre-existing groups plus `packaged-release-issues-180-183`, which the evidence guard grades as older than the tree because later commits touched the interface source after v1.0.2035; refresh from a current release |
...
| Local Docker destination | verified (2026-09-05) | Docker engine reachable in a later session; the released `worldlens-cli:1.0.2006` image rendered the 1 GB fixture to 60,426 tiles, see the Docker render row below |
